...
using LinearAlgebra: eigen, Hermitian, mul!
...
function (opt::ConstrainedRKOCOptimizer{T,E,P})() where {T,E,P}
    ...
    eig = eigen(Hermitian(opt.kkt_matrix))
    coeffs = eig.vectors' * opt.kkt_vector
    solution = zero(opt.kkt_vector)
    solution_norm = sum(abs2, opt.kkt_vector)
    for i in sortperm(eig.values; by=abs, rev=true)
        trial = solution + (coeffs[i] / eig.values[i]) * eig.vectors[:, i]
        trial_norm = sum(abs2, opt.kkt_matrix * trial - opt.kkt_vector)
        if trial_norm < solution_norm
            solution = trial
            solution_norm = trial_norm
        end
    end
    copy!(opt.kkt_vector, solution)
    ...
    while true
        ...
        if any(isnan, x_trial) || (x_trial == opt.x)
            return false
        end
